...
tf.flags.DEFINE_float('nbatch_kg', 100, 'entity numbers used each training time')
tf.flags.DEFINE_float('margin', 1.0, 'entity numbers used each training time')
tf.flags.DEFINE_float('learning_rate_kg', 0.001, 'learning rate for kg')
tf.flags.DEFINE_integer('ent_total', lib.getEntityTotal(), 'total of entities')
tf.flags.DEFINE_integer('rel_total', lib.getRelationTotal(), 'total of relations')
tf.flags.DEFINE_integer('tri_total', lib.getTripleTotal(), 'total of triples')
tf.flags.DEFINE_integer('katt_flag', 1, '1 for katt, 0 for att')

tf.flags.DEFINE_string('model', 'cnn', 'neural models to encode sentences')
...
tf.flags.DEFINE_integer('num_classes', config['textual_rel_total'], 'maximum of relations')

tf.flags.DEFINE_integer('hidden_size', 230, 'hidden feature size')
tf.flags.DEFINE_integer('pos_size', 5, 'position embedding size')

tf.flags.DEFINE_integer('max_epoch', 30, 'maximum of training epochs')
tf.flags.DEFINE_integer('batch_size', 131 * 2, 'entity numbers used each training time')
...
tf.flags.DEFINE_integer('test_batch_size', 131 * 2, 'entity numbers used each test time')
...
def main(_):
    ...
    fff = open('details_ori.txt', "w")
    ffx = open('details_out.txt', "w")
    for iters in range(1, 10):
        print(iters)
        saver.restore(sess, FLAGS.checkpoint_path + FLAGS.model + str(FLAGS.katt_flag) + "-" + str(3664 * iters))
        stack_output = []
        stack_label = []
        iteration = int(len(test_instance_scope) / FLAGS.test_batch_size)
        for i in range(iteration):
            ...
            for num in input_scope:
                index = index + [i for i in range(num[0], num[1] + 1)]
            ...
            output = test_step(test_head[index], test_tail[index], test_word[index, :], test_pos1[index, :],
                               test_pos2[index, :], test_mask[index, :], test_len[index], test_label[index], label_,
                               np.array(scope))
            print(output)
            stack_output.append(output)
            stack_label.append(label_)
            """KG supervision start here."""
        ...
        average_precision = average_precision_score(exclude_na_flatten_label, exclude_na_flatten_output,
                                                    average="micro")
        fff.write(str(exclude_na_flatten_label))
        ffx.write(str(exclude_na_flatten_output))
