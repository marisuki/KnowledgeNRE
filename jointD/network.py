...
class NN(object):
    ...
    def __init__(self, is_training, word_embeddings, simple_position=False):
        ...
        with tf.name_scope("embedding-layers"):
            # word embeddings
            print(FLAGS.ent_total)
            temp_word_embedding = tf.get_variable(initializer=word_embeddings[int(FLAGS.ent_total):, 0:],
                                                  name='temp_word_embedding', dtype=tf.float32)
            ent_embedding = tf.get_variable(name="ent_embedding", shape=[FLAGS.ent_total, self.word_size],
            ...
            self.relation_matrix = tf.get_variable('relation_matrix', [self.num_classes, self.output_size],
            ...
            self.transfer_bias = tf.get_variable('transfer_bias', [self.word_size], dtype=tf.float32,
                                                 initializer=tf.contrib.layers.xavier_initializer())
            self.ent_transfer = tf.get_variable(name="ent_transfer", shape=[FLAGS.ent_total, self.word_size],
                                                initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            self.rel_transfer = tf.get_variable(name="rel_transfer", shape=[FLAGS.rel_total, self.word_size],
                                                initializer=tf.contrib.layers.xavier_initializer(uniform=False))
    ...
    def katt_test(self, x, is_training=False):
        ...
        for i in range(self.num_classes):
            each_att = tf.expand_dims(
                self.calc(head, head_transfer, tf.reshape(self.rel_transfer[i], [-1, self.word_size])) -
                self.calc(tail,tail_transfer,tf.reshape(self.rel_transfer[i],[-1,self.word_size])),
                -1)
