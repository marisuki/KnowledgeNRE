...
tf.flags.DEFINE_integer('max_epoch', 20, 'maximum of training epochs')
tf.flags.DEFINE_integer('batch_size', 160, 'entity numbers used each training time')
tf.flags.DEFINE_float('learning_rate', 0.5, 'learning rate for nn')
    ...
    def train_nn(coord):
        ...
        for one_epoch in range(FLAGS.max_epoch):
            ...
            for i in range(int(len(train_order) / float(FLAGS.batch_size))):
                input_scope = np.take(instance_scope, train_order[i * FLAGS.batch_size:(i + 1) * FLAGS.batch_size],
                                      axis=0)
                index = []
                scope = [0]
                label = []
                ...
                output, loss, correct_predictions = train_step(train_head[index], train_tail[index],
                                                               train_word[index, :], train_pos1[index, :],
                                                               train_pos2[index, :], train_mask[index, :],
                                                               train_len[index], train_label[index], label_,
                                                               np.array(scope), weights)
                num = 0
                s = 0
                losstot += loss
                ...
                time_str = datetime.datetime.now().isoformat()
                print("batch %d step %d time %s | loss : %f, NA accuracy: %f, not NA accuracy: %f" % (
